...
```
ffmpeg -r 1 -colorspace bt709 -i Mian.mp4 -r 1 -colorspace bt709 -i Refs.mp4 -hide_banner -map_chapters -1 -map_metadata -1 -an -sn -dn -lavfi "[0:v:0]trim=start_frame=5000:end_frame=15000,pad=1920:1080:(ow-iw)/2:(oh-ih)/2[main];[1:v:0]trim=start_frame=5000:end_frame=15000,pad=1920:1080:(ow-iw)/2:(oh-ih)/2[refs];[main][refs]libvmaf=model=version=vmaf_v0.6.1:n_threads=16:log_fmt=csv:log_path=Main.csv" -f null -
```
...
```
ffmpeg -filter_complex ddagrab=framerate=60,hwdownload,format=bgra,scale=out_color_matrix=bt709:out_range=tv:flags=accurate_rnd+full_chroma_int+bitexact -c:v h264_amf -profile:v high -quality balanced -qp_i 12 -qp_p 14 YYY.mp4
```
