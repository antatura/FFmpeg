...
```
ffmpeg -filter_complex ddagrab=framerate=60:dup_frames=0,hwdownload,format=bgra,scale=out_color_matrix=bt709:out_range=tv:flags=accurate_rnd+full_chroma_int+bitexact -c:v h264_amf -profile:v high -quality balanced -qp_i 12 -qp_p 14 YYY.mkv
```
...
```
ffmpeg -i XXX.mp4 -vf select='eq(n\,12)',zscale,format=gbrp,format=rgb24 YYY.png
```
